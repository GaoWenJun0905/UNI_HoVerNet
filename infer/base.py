...
# 20260308_GWJ_ConVNeXt_Base
    def __load_model(self):
        ...
        # 20260205_GWJ: 加载自定义的 UNI_HoVerNet 定义
        model_desc = import_module("models_GWJ.ConVNeXt_Base_TripMTCA_hovernet.net_desc_ConVNeXt_Base_MTCA")
        model_creator = getattr(model_desc, "create_model")
        ...
        # 20260205_GWJ: 加载对应的推理逻辑和后处理
        module_lib = import_module("models_GWJ.ConVNeXt_Base_TripMTCA_hovernet.run_desc")
        run_step = getattr(module_lib, "infer_step")
        self.run_step = lambda input_batch: run_step(input_batch, net)

        module_lib = import_module("models_GWJ.ConVNeXt_Base_TripMTCA_hovernet.post_proc")
        self.post_proc_func = getattr(module_lib, "process")
        return
