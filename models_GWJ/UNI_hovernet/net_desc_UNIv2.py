...
####
# 20260204_GWJ: 定义 UNI2 适配器 (针对 ViT-L/14 设计)
class UNI2_Adapter(nn.Module):
    """
    将 UNI2 (ViT-L/14) 的 16x16 特征图转换为 ResNet 风格的金字塔特征 (d0, d1, d2, d3)。
    UNI2 输出尺度固定为 1/14。
    """
    ...
    def __init__(self, embed_dim=1024):
        ...
        self.up_d0 = nn.Sequential(
            nn.ConvTranspose2d(embed_dim, 512, kernel_size=3, stride=2, padding=1, output_padding=1),  # 16->32
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
            nn.Upsample(size=(56, 56), mode='bilinear', align_corners=False),  # 32->56
...
####
class HoVerNet(Net):
    """Initialise HoVer-Net with UNI2 Backbone."""
    ...
    def __init__(self, input_ch=3, nr_types=None, freeze=False, mode='original'):
        ...
        # --- 20260204_GWJ: 使用 timm 加载 UNI2 (DINOv2-L/14) ---
        # 这里的 model_name 需对应 timm 中的实际名称，或通过 pretrained_cfg_overlay 加载本地 bin
        self.backbone = timm.create_model(
            "vit_large_patch14_dinov2.lvd142m",
            pretrained=False,  # 设为 False，由外部手动加载 UNI2 权重
            img_size=224,
            num_classes=0,
        ...
        # 实例化 UNI2 专用适配器
        self.adapter = UNI2_Adapter(embed_dim=1024)
        # --------------------------------------------------
        ...
        def create_decoder_branch(out_ch=2, ksize=5, is_tp=False):
            ...
            if is_tp:
                module_list_u1.append(("se", SEModule(64)))
    ...
    def forward(self, imgs):
        ...
        with torch.set_grad_enabled(not self.freeze):
            # 20260204_GWJ: 使用 timm 获取中间层 (DINOv2)
            # DINOv2-L 有 24 层，我们取 7, 15, 23, 23
            indices = [7, 15, 23, 23]
            feats_tokens = self.backbone.get_intermediate_layers(imgs_input, n=indices, reshape=True)
            # timm 的 reshape=True 会自动把 [B, N, C] 转为 [B, C, H, W]

            d0, d1, d2, d3 = self.adapter(feats_tokens)
